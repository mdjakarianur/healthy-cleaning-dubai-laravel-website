...
<div class="content">
    ...
    <!-- Active Link -->
    @slot('active_link')
    @lang('photo_gallery.index_title')
    @endslot
    ...
    @if(Auth::user()->can('Photo Gallery trash'))
    <!-- button two -->
    ...
    {{route('photo_gallery.trash_list')}}
    @endslot
            ...
            <div class="table-responsive">
                <table class="table myTable  fs--1 mb-0">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>@lang('photo_gallery.title')</th>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    $(".myTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('photo_gallery.index') }}",
        columns: [
            {data: 'sl', name: 'sl'},
...
<script>
    function changePhotoGallerysStatus(id)
    {
        // alert(id);
        $.ajax({
            headers : {
