...
class ServiceCategoryRepository implements ServiceCategoryInterface
{
    ...
    {
        try {
            $data = ServiceCategory::withTrashed()->findOrFail($id);

            // check service exists or not
            if ($data->services()->count() > 0) {
                toastr()->error('This category has services. Delete them first.', 'Error');
                return redirect()->back();
            }
            ...
            toastr()->success(__('service_category.delete_message'), __('common.success'));
            return back();
        } catch (\Throwable $th) {
